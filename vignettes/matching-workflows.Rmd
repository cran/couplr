...
vignette: >
  %\VignetteIndexEntry{Matching Workflows}
...
- Understanding of causal inference concepts (treatment effects, confounding)

- Basic statistics (means, standard deviations, t-tests)

**What You'll Learn**:

- How to match treatment and control units with `match_couples()`

- Automatic preprocessing: scaling, health checks, categorical encoding

- Assessing match quality with `balance_diagnostics()`
...
```{r inspect-output, fig.alt="Histogram showing the distribution of match distances, with most matches having low distances indicating good match quality"}
# How many matched?
...
- Use `strategy = "sorted"` when quality is important but optimal is too slow
...
```{r calipers, fig.alt="Overlapping histograms comparing match distances with and without caliper constraint, showing the caliper excludes distant matches"}
...
```{r balance-plots, fig.alt="Bar chart comparing standardized differences before and after matching, with threshold lines at 0.1 and 0.25 showing improved balance after matching"}
...
```{r optimization-blocking, eval=FALSE}
...
```{r optimization-greedy, eval=FALSE}
...
```{r optimization-caliper, eval=FALSE}
...
**Causes**:
- Groups are fundamentally too different (weak overlap)
...
**Diagnosis**:
```{r few-matches-diagnosis, eval=FALSE}
...
**Symptom**: `match_couples()` runs for minutes or doesn't complete.
**Cause**: $O(n^3)$ complexity for optimal matching.

**Solutions**:
```{r slow-matching-fix, eval=FALSE}
...
**Solutions**:
- Use `greedy_couples()` which doesn't require full matrix
...
```{r workflow-diagram, fig.width=8, fig.height=6, echo=FALSE, fig.alt="Flowchart showing recommended matching workflow with iterative refinement loop"}
library(ggplot2)
...
Standard one-to-one matching discards unmatched units — a significant loss when control pools are large. Full matching assigns *every* unit to a matched group with variable ratios (1:k or k:1), maximizing sample size while preserving balance.
...
The `method` argument controls the algorithm:

- `"optimal"` (default) — Solves a min-cost max-flow problem that minimizes total distance across all group assignments simultaneously. Uses Dijkstra's algorithm with Johnson potentials. Guarantees the globally optimal solution.

- `"greedy"` — A fast two-pass heuristic: (1) each left unit picks its nearest right unit, (2) remaining right units are assigned to their nearest matched left unit. Faster but not globally optimal.

```{r full-matching-comparison}
...
Full matching supports `min_controls` and `max_controls` to constrain group sizes, and `caliper` to exclude poor matches:

```{r full-matching-constraints}
...
The output is an S3 object of class `full_matching_result` with a `groups` tibble containing `group_id`, `id`, `side`, and `weight`. Within each group, right-unit weights sum to the number of left units, enabling unbiased estimation:

```{r full-matching-weights}
head(result_full$groups, 10)
```

**When to use full matching vs one-to-one:**

| Criterion | One-to-one (`match_couples`) | Full (`full_match`) |
|-----------|------------------------------|---------------------|
| Sample size | Discards excess units | Uses every unit |
| Estimand | ATT (treatment on treated) | ATT or ATE (with weights) |
| Group structure | Pairs (1:1) | Variable-ratio groups |
| Balance | Excellent within pairs | Good overall (weighted) |
| Precision | Lower (smaller n) | Higher (larger effective n) |

### Ratio Matching (k:1)

Sometimes matching multiple controls per treated unit improves precision. Use the `ratio` parameter:
...
Higher ratios increase sample size at the cost of slightly worse individual match quality. A common choice is 2:1 or 3:1.

### With-Replacement Matching

When controls are scarce relative to treated units, allowing replacement lets each control serve as a match for multiple treated units:

```{r replacement-matching}
...
With-replacement matching reduces bias (each treated unit gets its closest match) but increases variance.

### Propensity Score Matching

Instead of matching directly on covariates, you can match on the estimated probability of treatment — the propensity score. `ps_match()` automates this:
...
The function fits a logistic regression, transforms scores to the logit scale, and calls `match_couples()` with a caliper constraint. The default caliper is 0.2 SD of logit(PS) (Rosenbaum and Rubin recommendation); we use 0.5 here for this small simulated dataset. You can also supply a pre-fitted model via the `ps_model` argument.

### Cardinality Matching

Standard matching minimizes total distance. Cardinality matching instead maximizes the number of matched pairs while enforcing strict balance constraints:

```{r cardinality-matching}
...
This is useful when you need strict balance guarantees and are willing to sacrifice some sample size.

### Coarsened Exact Matching (CEM)

CEM coarsens continuous variables into bins, then performs exact matching within strata. This avoids model dependence — results don't rely on a distance function or propensity score model:
...
You can supply custom cutpoints per variable with the `cutpoints` argument, or pass categorical grouping variables with `grouping`. Each matched unit receives a weight inversely proportional to the stratum size, ensuring balance across strata.

### Subclassification

Subclassification (stratification on the propensity score) divides the sample into strata of similar PS, then computes within-stratum effects. Unlike one-to-one matching, no units are discarded:
...
All couplr result types can be converted to a unified analysis-ready format or to MatchIt-class objects for interoperability with cobalt and marginaleffects.

**`match_data()`** produces a single data frame with `treatment`, `weights`, `subclass`, and `distance` columns — ready for downstream modelling:

```{r match-data-example}
md <- couplr::match_data(result, left_data, right_data)
head(md)
```

**`as_matchit()`** constructs a `matchit`-class object, enabling direct use with `cobalt::bal.tab()`, `cobalt::love.plot()`, and `marginaleffects::avg_comparisons()`:
```{r as-matchit-example, eval=FALSE}
...
After matching, a natural question is: *how much hidden bias would be needed to invalidate the results?* Rosenbaum bounds address this by computing p-values under varying degrees of unobserved confounding.
...
The **critical gamma** ($\Gamma$) is the smallest value at which the upper-bound p-value crosses 0.05. A larger critical gamma means the finding is more robust: a critical gamma of 1.75, for example, means an unobserved confounder would need to change the odds of treatment by a factor of 1.75 to explain away the result.

---

## Visualizing Results with autoplot

If `ggplot2` is available, `autoplot()` methods produce publication-ready plots:

```{r autoplot-examples, fig.width=7, fig.height=4}
...
```{r autoplot-balance, fig.width=7, fig.height=4}
if (requireNamespace("ggplot2", quietly = TRUE)) {
...
```{r autoplot-sensitivity, fig.width=7, fig.height=4}
if (requireNamespace("ggplot2", quietly = TRUE)) {
...
All `autoplot()` methods return `ggplot` objects, so you can add layers, themes, and labels as needed.

---

## See Also

- `vignette("getting-started")` - Basic LAP solving

- `vignette("algorithms")` - Mathematical foundations
...
- `?match_couples`, `?greedy_couples`, `?full_match`, `?balance_diagnostics`, `?matchmaker`

- `?ps_match`, `?cardinality_match`, `?sensitivity_analysis`, `?cem_match`, `?subclass_match`

- `?match_data`, `?as_matchit` — ecosystem integration
